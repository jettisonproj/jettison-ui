...
function isMemoizedNode(nodeType: NodeType) {
  switch (nodeType) {
    ...
    default:
      ...
      console.log("unexpected node type in workflowk");
...
function isWorkflowGraphNode(node: WorkflowStatusNode) {
  return isMemoizedNode(node.type) && !node.displayName.endsWith(".onExit");
}
...
export { isMemoizedNode, isWorkflowGraphNode };
